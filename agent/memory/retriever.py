...
class CypherRetriever(BasePGRetriever):
    """A retriever that uses a language model to generate Cypher queries.
    ...
    def __init__(
        ...
        allowed_output_fields: Optional[List[str]] = DEFAULT_ALLOWED_FIELDS,
        **kwargs: Any,
    ) -> None:
        ...
        self.text_to_cypher_template = (
            text_to_cypher_template or load_prompt_template("cypher_prompt")
        )
    ...
    def retrieve_from_graph(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        messages = [
            ChatMessage(
                ...
                content=self.text_to_cypher_template.format(schema=schema)
            ),
            ChatMessage(
                role=MessageRole.USER,
                content=f"given the provided schema, please generate a cypher query to answer the following question: {question}"
            )
        ]
        response = self.llm.chat(messages)

        parsed_cypher_query = str(response.message.content)
        if self.allowed_output_fields is not None:
            parsed_cypher_query = self._parse_generated_cyher(parsed_cypher_query)

        try:
            query_output = self._graph_store.structured_query(parsed_cypher_query)
        except Exception as e:
            print(f"Error executing cypher query: {e}")
            print(f"Cypher query: {parsed_cypher_query}")
            return []
